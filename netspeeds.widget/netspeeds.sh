...
myvar1=`netstat -ibn | grep -e "en1" -m 1 | awk '{print $7}'` #  bytes in
myvar3=`netstat -ibn | grep -e "en1" -m 1 | awk '{print $10}'` # bytes out

# AIRPORT: get the current number of bytes in and bytes out
apmyvar1=`netstat -ibn | grep -e "en0" -m 1 | awk '{print $7}'` #  bytes in
apmyvar3=`netstat -ibn | grep -e "en0" -m 1 | awk '{print $10}'` # bytes out

#wait one second
sleep 1

# ETHERNET: get the number of bytes in and out one second later
myvar2=`netstat -ibn | grep -e "en1" -m 1 | awk '{print $7}'` # bytes in again
myvar4=`netstat -ibn | grep -e "en1" -m 1 | awk '{print $10}'` # bytes out again

# AIRPORT: get the number of bytes in and out one second later
apmyvar2=`netstat -ibn | grep -e "en0" -m 1 | awk '{print $7}'` # bytes in again
apmyvar4=`netstat -ibn | grep -e "en0" -m 1 | awk '{print $10}'` # bytes out again
...
echo "$kbin#$kbout#$apkbin#$apkbout#$airip#$etherip"
